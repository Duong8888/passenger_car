...
<body class="bg-white dark:bg-neutral-800">
    <section class="py-20 bg-gray-50 dark:bg-neutral-700">
            ...
            <div class="flex justify-between">
                ...
                <div class="w-2/5 "  style="max-width: 37%; min-width: 37%;">
                    <div class="mb-5 border p-4">
                        ...
                        <p class="mb-3">{{ isset(session('value')[0]['username']) ? session('value')[0]['username'] : Auth::user()->name }}</p>
        ...
        <div class="fixed bottom-0 left-0 right-0 p-4 bg-white shadow-md flex justify-center offline-ticket hidden">
            <button data-action="{{ route('client.ticket.end-payment-ticket') }}" data-session="{{ json_encode(session('value')) }}" class="bg-yellow-500 w-1/2 p-2 m-2">Thanh toán tại nhà xe</button>
          ...
          <form action="{{ route('client.ticket.vnpay-method') }}" method="POST" class="fixed bottom-0 left-0 right-0 p-4 bg-white shadow-md flex justify-center vnpay-ticket hidden">
            @csrf
            <button name="redirect" class="bg-yellow-500 w-1/2 p-2 m-2" id="payment-vnp" type="submit">Thanh toán VNPay</button>
