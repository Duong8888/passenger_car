...
class TicketController extends Controller
{
    public function CountTicket(Request $request)
    {
        ...
        if($check){
            ...
            session()->forget('value');
            session()->flash('message', 'Ghế của bạn đã có người nhanh tay hơn đặt rồi vui lòng chọn gế khác.');
            return response()->json(['error' => $request->all()], Response::HTTP_OK);
        }
    }
    ...
    public function checkoutPayment(Request $request)
    {
        if (session('value')) {
            ...
            if ($request->vnp_ResponseCode == '00' && $request->vnp_TransactionStatus == '00') {
                ...
                $car = PassengerCar::where('id', $ticket->passenger_car_id)->get();
                $layout = SeatsLayout::where('vehicle_id', $car[0]->vehicle_id)->get();
                SendMail::dispatch(session('value')[0]['email'], $ticket, $layout);
