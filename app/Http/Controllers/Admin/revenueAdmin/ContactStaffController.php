...
class ContactStaffController extends Controller
{
    ...
    public function filter_by_date(Request $request){
        ...
        $tickets = Contact::whereBetween('created_at', [$form_date, $to_date])->orderBy('created_at', 'ASC')->get();
        $chart_data = [];
    ...
    //    echo $today = Carbon::now('Asia/Ho_Chi_Minh')->format('d-m-Y H:i:s');
       ...
       if($data['dashboard_value'] == '7ngay'){
        $tickets = Contact::whereBetween('created_at', [$sub7days, $now])->orderBy('created_at', 'ASC')->get();
       }elseif($data['dashboard_value'] == 'thangtruoc'){
        $tickets = Contact::whereBetween('created_at', [$dau_thangtruoc, $cuoi_thangtruoc])->orderBy('created_at', 'ASC')->get();
       }elseif($data['dashboard_value'] == 'thangnay'){
        $tickets = Contact::whereBetween('created_at', [$dauthangnay, $now])->orderBy('created_at', 'ASC')->get();
       }else{
    ...
    public function dayrevenue(Request $request)
    {
        ...
        $now = Carbon::now('Asia/Ho_Chi_Minh')->toDateString();
        $tickets = Contact::whereBetween('created_at', [$sub60days, $now])->orderBy('created_at', 'ASC')->get();
        $chart_data = [];
        foreach ($tickets as $val) {
            $date = Carbon::parse($val->created_at)->toDateString();
            $status = $val->status;
